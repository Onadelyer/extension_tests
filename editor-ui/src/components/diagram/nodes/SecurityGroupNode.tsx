...
import { SecurityGroupIcon } from '../../../assets/aws-icons';
import useDiagramStore from '../../../store/diagramStore';

// Style for Security Group node
const nodeStyle = {
  padding: '8px',
  borderRadius: '4px',
  width: '120px',
...
const SecurityGroupNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <SecurityGroupIcon color="#FF9900" size={24} />
        <div style={{ fontWeight: 'bold', marginTop: '4px', textAlign: 'center', maxWidth: '100px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {data.name || 'Security Group'}
        </div>
        <div style={{ fontSize: '10px', color: '#666', marginTop: '2px' }}>
          {data.description || 'Security rules'}
        </div>
      </div>
...
export default memo(SecurityGroupNode); 